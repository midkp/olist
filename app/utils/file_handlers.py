...
def validate_csv_structure(table_name: str, file):
    required_columns = {
        ...
        'leads_qualified': ['mql_id', 'first_contact_date', 'landing_page_id', 'origin'],
        'leads_closed': ['mql_id', 'seller_id', 'sdr_id', 'sr_id', 'won_date', 'business_segment', 'lead_type', 'lead_behaviour_profile', 'has_company', 'has_gtin', 'average_stock', 'business_type', 'declared_product_catalog_size', 'declared_monthly_revenue'],
        'product_category_name_translation': ['product_category_name', 'product_category_name_english']
    }
    
    reader = csv.DictReader(file.file.read().decode('utf-8-sig').splitlines())  # Changed to utf-8-sig
    print(f"Expected columns: {required_columns.get(table_name, [])}")
    print(f"Actual columns: {reader.fieldnames}")
