...
class CSVService:
    """Handles CSV file processing and database insertion."""

    @staticmethod
    async def process_csv_upload(table_name: str, file, chunk_size: int = 1000):
        """Process a CSV file and insert data into the specified table."""
        try:
            # Define a controlled mapping of allowed table names to prevent SQL injection
            table_mapping = {
                "customers": "customers",
                "orders": "orders",
            ...
            if full_table_name not in table_mapping.values():
            ...
            with database.get_db_connection() as conn:
                ...
                df = pd.read_csv(
                    ...
                    dtype=dtypes.get(full_table_name, {}),
                    encoding="utf-8-sig",
                )
                    ...
                    for chunk in df:
                        ...
                        for _, row in chunk.iterrows():
                            try:
                                ...
                                # S608 is safe here due to table_mapping validation
                                cursor.execute(
                                    f"SELECT {primary_key} FROM {full_table_name} "  # noqa: S608
                                    f"WHERE {primary_key} = ?",
                                    (row[primary_key],),
                                )
                                if cursor.fetchone():
                                ...
                                # S608 is safe here due to table_mapping validation
                                cursor.execute(
                                    f"INSERT INTO {full_table_name} ({columns_str}) "  # noqa: S608
                ...
                conn.commit()
                logger.info(
                    f"Inserted {inserted} rows, skipped {skipped} rows into table {full_table_name}"
                )
